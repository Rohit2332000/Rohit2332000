# Hi, I'm Rohit 👋

I'm an **AI Engineer** focused on building practical and production-ready
**Generative AI, Agentic AI, and RAG applications**.

I enjoy building AI systems that combine **LLMs, retrieval, agents, tools,
and evaluation** to solve real-world problems.

## What I work on

- 🤖 AI Agents, Multi-Agent Systems & MCP
- 🔎 Advanced RAG, Vector Search & Graph RAG
- 🧠 LLM Applications & Prompt Engineering
- 📊 RAG / LLM Evaluation with LangSmith & DeepEval
- ⚡ Python, FastAPI & PostgreSQL
- ☁️ AWS, Docker, Kubernetes & CI/CD

## About Me

- 🚀 Building production-ready GenAI applications
- ✈️ Built an AI-powered Aircraft Maintenance Intelligence System
- 🤖 Built Agentic AI systems with LangGraph & MCP
- 🔎 Experienced in RAG, tool calling and LLM evaluation
- 🏏 Enjoy playing cricket and exploring AI
- 💼 **Open to AI/ML & Generative AI opportunities**

## Featured Projects

- 🤖 [MCP Powered Assistant](https://github.com/rohit2332000/mcp-powered-assistant)
- 📊 [RAG Evaluation using DeepEval](https://github.com/rohit2332000/RAG_EVALUATION_using_DeepEVAL)
- ✈️ [Aircraft Maintenance Intelligence](https://github.com/rohit2332000/aircraft-maintenance-intelligence)
- 🌍 [Agentic Trip Planning](https://github.com/rohit2332000/agentic-trip-planning-system)
- 🖼️ [Multimodal AI System](https://github.com/rohit2332000/Multimodal-AI-system)
- ✍️ [Agentic Blogger](https://github.com/rohit2332000/Agentic-Blogger)

## 🌐 Connect

- 🌍 [Portfolio](https://rohit2332000.github.io/rohityadav.github.io/)
- 💻 [GitHub](https://github.com/rohit2332000)
- 💼 [LinkedIn](https://www.linkedin.com/in/rohit-kumar-yadav-42223a303/)
- 📧 [Email](rohitrana2332000@gmail.com)

---

**Building AI systems from prototype → production 🚀**
